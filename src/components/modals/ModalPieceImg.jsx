...
import { GET_POST_PIECE_IMG, postPieceImage } from '../../redux/action/pieces'

const ModalPieceImg = ({ showImgModal, setShowImgModal, pieceId }) => {
  ...
  const handleUploadImage = async (id) => {
    try {
      ...
      if (formImg) {
        ...
        if (id_piece) {
          const response = await postPieceImage(id_piece, formImg, token)
          console.log('response', response)
          if (response !== null) {
            console.log('Immagine caricata correttamente', response)
            console.log('id pezzo', id)
            console.log('altro id pezzo', id_piece)
            dispatch({
