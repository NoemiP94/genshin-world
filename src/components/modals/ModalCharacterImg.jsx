...
const ModalCharacterImg = ({ showImgModal, setShowImgModal, characterId }) => {
  ...
  const handleUploadImage = async (id) => {
    try {
      console.log('cliccato'), console.log('id personaggio', id)
      ...
      if (formImg) {
        ...
        if (id_character) {
          ...
          if (response !== null) {
            ...
            console.log('id personaggio', id)
            console.log('altro id personaggio', id_character)
            dispatch({
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-700 outline-none focus:outline-none">
            ...
            <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
              <button
                ...
                type="button"
                onClick={() => handleSave(characterId)}
              >
                Salva
              </button>
            </div>
...
export default ModalCharacterImg
