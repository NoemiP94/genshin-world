...
const ModalConstellationImg = ({
  ...
  const handleUploadImage = async (id) => {
    try {
      console.log('cliccato'), console.log('id constellation', id)
      console.log(constellationId)

      if (formImg) {
        console.log('formImg', formImg)
        const id_constellation = id ? id.toString() : null
        console.log('id_constellation: ', id_constellation)
        if (id_constellation) {
          ...
          if (response !== null) {
            ...
            console.log('altro id constellation', id_constellation)
            dispatch({
          ...
          console.log('Constellation ID not recovered')
        }
      }
    } catch (error) {
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-700 outline-none focus:outline-none">
            ...
            <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
              <button
                ...
                type="button"
                onClick={() => handleSave(constellationId)}
              >
                Salva
              </button>
            </div>
...
export default ModalConstellationImg
