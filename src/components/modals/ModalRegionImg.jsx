  ...
  const handleUploadImage = async (id) => {
    try {
      ...
      if (formImg) {
        ...
        const id_region = id ? id.toString() : null
        console.log('id_region: ', id_region)
        if (id_region) {
          const response = await postRegionImage(id_region, formImg, token)
          console.log('response', response)
          if (response !== null) {
            console.log('Immagine caricata correttamente', response)
            console.log('id regione', id)
            console.log('altro id regione', id_region)
            dispatch({
